...
struct LFSRNoise[num_chans: Int = 1](Copyable, Movable):
    ...
    def set_oversampling(mut self, times_oversampling: TimesOversampling):
        """Sets times oversampling for the oscillator when it is used in an Downsampler loop. This is not for when using the oscillator with the built-in oversampling, but rather for when the oscillator is used as part of a custom oversampling implementation.
